...
export const ProductDescription = ({ selectedProduct }) => {
  const { addToCartHandler, wishlistHandler, isProductInCart, isProductInWishlist, cartLoading } = useUserData();
  ...
  return (
    <div className="product-details-description">
      ...
      <div className="ratings-reviews">
        <span></span>
        <span>{selectedProduct?.rating}</span> <BsFillStarFill color={"orange"} />
        <span>
      ...
      <div className="tags">
        {!selectedProduct?.is_stock && <span className="out-of-stock">{selectedProduct?.is_stock ? "In Stock" : "Out of stock"}</span>}
        {selectedProduct?.trending && <span className="trending">{selectedProduct?.trending ? "Trending" : ""}</span>}
      </div>
      <div className="product-card-buttons-container">
        <button disabled={cartLoading} onClick={() => addToCartHandler(selectedProduct)} className="add-to-cart-btn">
          {!isProductInCart(selectedProduct) ? "Add to cart" : "Go to cart"}
        </button>
        <button disabled={cartLoading} onClick={() => wishlistHandler(selectedProduct)} className="add-to-wishlist-btn">
